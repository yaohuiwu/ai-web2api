...
> Design docs: [`docs/DESIGN.md`](docs/DESIGN.md) (§3 covers provider extension points); Qwen in [`docs/PROVIDER_QWEN.md`](docs/PROVIDER_QWEN.md); ChatGPT in [`docs/PROVIDER_CHATGPT.md`](docs/PROVIDER_CHATGPT.md); Kimi in [`docs/PROVIDER_KIMI.md`](docs/PROVIDER_KIMI.md); auth expiry in [`docs/AUTH_EXPIRY.md`](docs/AUTH_EXPIRY.md). Deferred work and its rationale: [`docs/TODO.md`](docs/TODO.md). **DeepSeek / ChatGPT are enabled by default** (Qwen is wired up but `enabled: false`; turn it on when needed).
...
| **Gemini** | ✅ **Works — guest mode, no login needed** | `gemini-web` | guest (Google sign-in optional) | Verified end to end (15.8s). The composer is a Quill editor and replies are matched via `.model-response-text` (user turns are `<user-query>`, so they are never mistaken for answers). When the free quota runs out the site **degrades to Flash-Lite and keeps working** — extraction is model-agnostic, so nothing changes for us ([`docs/PROVIDER_GEMINI.md`](docs/PROVIDER_GEMINI.md)) |
...
| **GLM (Zhipu)** | ⚠️ **Disabled by default — Aliyun WAF blocks automation** | `glm-web` | **manual only** (phone/SMS or QR) | chatglm.cn fronts an **Aliyun WAF slider** that blocks headless *and* headful automation, and its ticket lives only **~30 minutes** (measured), so it needs a human re-verification every half hour — the account itself is *not* logged out (login cookies last 30 days). Selectors are calibrated and it works during that window; enable it temporarily if needed, but prefer Zhipu's official API for anything long-running ([`docs/PROVIDER_GLM.md`](docs/PROVIDER_GLM.md)) |
...
> in front (see [`docs/LIVE_CONTROL.md`](docs/LIVE_CONTROL.md)).
