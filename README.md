...
- **Login state that survives restarts** — `storage_state` persistence (rotated cookies **and localStorage tokens** are fingerprinted and re-saved), automatic login, one-command manual login, state import from the UI, and an **auth-expiry countdown + reminder** for manual providers
...
- **Live view (read-only)** — watch the provider's real browser from the UI (`/ui/browser.html`, MJPEG at 5 fps) — it follows the session you're actively chatting in, and any thread card links straight to its own view (`?thread_id=`); frames fan out to all viewers, stop when nobody watches, and drop to 1 fps while a request is in flight. **Unauthenticated by design for now** — do not expose it publicly (`server.live_view`)
...
| **Doubao** | 🧪 **Scaffold — manual login + calibration needed** | `doubao-web` | **manual only** (no password login; the container shows a region restriction until you sign in) | Composer (`tiptap/ProseMirror`) and message containers are measured; send button / response container / login check still need calibration after login ([`docs/PROVIDER_DOUBAO.md`](docs/PROVIDER_DOUBAO.md)) |
| **GLM (Zhipu)** | ⚠️ **Blocked by Aliyun WAF slider** | `glm-web` | **manual only** | chatglm.cn serves a WAF "slide to verify" page to **both** headless and headful browsers, so DOM automation cannot get through. Workaround (fragile): pass it manually in a visible browser on the host, then import the state ([`docs/PROVIDER_GLM.md`](docs/PROVIDER_GLM.md)) |
