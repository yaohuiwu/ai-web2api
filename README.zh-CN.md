...
> **文本捕获保真度**（2026-09-22 全量实测）：长文本、代码块、Markdown 表格三类用例下，
> **DeepSeek / ChatGPT / Kimi / 豆包的句子覆盖率均为 1.00、零思考污染（12/12 用例）**；
> GLM 因阿里云 WAF 票据（实测约 30 分钟 TTL）过期而跳过。度量方法、原始结果、回归阈值与踩到的两个坑见
> [`docs/TEXT_FIDELITY.md`](docs/TEXT_FIDELITY.md)（`./scripts/text_fidelity.sh`）。
...
browser:
  ...
  auth_expiry_warn_days: 3   # 「认证即将过期」默认提醒阈值（天）；provider 可用 login.expiry_warn_days 覆盖
...
providers:                 # 也支持 list 写法
  deepseek:                # ← provider 名（同时决定驱动类）
    ...
    login:
      ...
      auth_cookies: []            # 代表登录态的 cookie（glob）→ 「认证有效期」展示/提醒
      # expiry_warn_days: 7       # 手动认证 provider 可提前提醒（见 config.yaml 的 providers.chatgpt）
...
| POST | `/admin/{p}/login/state` | 导入完整 `storage_state`（cookies + localStorage），写盘 + 重置 context，**立即生效** |
...
| GET | `/admin/threads` | 会话绑定：会话列表（活跃 + 已落库）；支持 `q` / `provider` / `limit` / `offset` / `order` |
...
├── cli.py             # `ai-web2api login|providers`（手动登录 + 导入登录态）
├── api/               # OpenAI 兼容路由、schema、SSE
├── browser/           # 浏览器管理（单实例多 Context + storage_state 持久化）、DOM→Markdown 提取
├── providers/         # 驱动基类（WebChatProvider 通用引擎）+ DeepSeek/Qwen/ChatGPT 实现 + 注册表
├── core/              # SerialGate、错误类型、ThreadManager、SQLite 历史（store.py）、认证有效期
├── tool_calling/      # Function Calling：prompt 注入 / 输出解析 / 消息转录
└── webui/             # 状态面板 + Playground + 会话页，HTML/CSS/JS 已分离
    ├── index.html / playground.html / threads.html   # 只留结构，引用下方 assets
    └── assets/{css,js}/                              # tokens + base + 各页面 css/js；common.js 为公共工具
scripts/
└── login.sh           # 一条命令手动登录（开浏览器 → 自动导入服务）
```
