...
| **豆包 Doubao** | 🧪 **脚手架 —— 需手动登录 + 待校准** | `doubao-web` | **仅手动**（无密码登录；容器内未登录会提示「受区域限制」） | 输入框（`tiptap/ProseMirror`）与消息容器已实测；发送按钮 / 正文容器 / 登录判定需登录后校准（[`docs/PROVIDER_DOUBAO.md`](docs/PROVIDER_DOUBAO.md)） |
