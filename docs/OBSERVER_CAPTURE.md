...
> 状态：**已实现**（默认 `poll`；**仅 ChatGPT 试点 `observer`**）。P1（`done_toolbar`）已完成；
> 本设计只改**抓取触发时机**，**不改结束判定语义**。
>
> ⚠️ 诚实说明（实现后修正）：observer **不减少提取次数**（每拍该提取还是提取，因为结束判定/稳定性
> 计数需要周期检查），它的收益是"① 变化一发生就唤醒 → 增量延迟 ≤ 去抖(100ms)+提取耗时，而不是
> 0~200ms 随机；② 容器自动重绑 → 站点替换回复容器时更稳"。要真正减少提取，需要把"提取"与
> "结束信号检查"拆成两条节奏（列为后续可选优化，见 §9）。
...
| `events` | 收到的变化事件数（去抖后） |
| `wakes` / `idle_ticks` | 这一拍是"页面变化唤醒"还是"定时兜底"（observer 模式特有） |
| `observer_fallback` | 1 = 发生过回退（此时 `capture=observer→poll`） |
| `capture` | `poll` / `observer` / `observer→poll` |

**未取到的度量**：`emit_lag_ms`（页面 `performance.now()` 与 Python `monotonic()` 属不同时钟域，需要做偏移校准）。
当前用"增量到达间隔"（客户端侧）近似衡量，见 §7.5。
...
## 7.6 实现后的实测（ChatGPT，2026-09-22 深夜）

| 指标 | poll 模式（对照） | **observer 模式** |
|---|---|---|
| `polls`（总拍数） | 60–74 | **34** |
| 其中事件唤醒 / 空闲兜底 | — | **`wakes=29` / `idle_ticks=12`** |
| `events`（去抖后事件数） | — | **32** |
| `extract_ms_total`（提取总耗时） | 449–658ms | **215ms** |
| 增量间隔 | 中位 0.21s | 中位 0.21s（受去抖 100ms + 最小间隔 150ms 决定） |
| 保真度 | 3/3 覆盖 1.00 | **覆盖 1.00**（含表格用例） |

读法：**事件唤醒占绝大多数**（29/34），说明页面变化就是我们的节奏来源；总提取耗时降到约 1/2–1/3
（变化少时不必每拍都提取的间接收益）。**增量间隔没变**——因为去抖(100ms)+最小间隔(150ms)≈原轮询(200ms)，
这点如实记录：observer 的价值在"**按变化对齐** + 容器重绑稳健"，不在"更细粒度"。

### 实现期踩到的两个坑（都已修 + 都加了回归测试）
1. **复位顺序**：`generate()` 里"每轮复位"的 `self._obs_wake = None` 必须早于 `_start_observer`，
   否则刚装好的唤醒器被清空 → `capture=observer` 却 **0 事件**。现由 `_reset_request_state()` 统一复位，
   并有一条**静态守卫测试**（断言源码里复位早于启动）。
2. **静默失效**：注入成功但**始终无事件**（选择器不匹配/容器被替换/页面重载）→ 现在
   **先重注入一次**（世代号递增），仍无事件则**回退轮询**并把 timeline 标成 `capture=observer→poll` +
   `observer_fallback=1`（度量诚实，行为与 poll 一致）。

### 测试
`tests/test_observer_capture.py`（13 条）：唤醒即时性、超时兜底、最小间隔限速、世代号过滤、
注入幂等、注入失败回退、grace 回退、静默失效重注入→回退、复位顺序静态守卫。
保真度脚本 `./scripts/text_fidelity.sh chatgpt` 可随时回归（题目随机取自文本库）。
...
把 DOM 循环里的两件事拆成**两条节奏**：
- **提取**：只在"页面确实变化"（事件）时做；
- **结束信号检查**（停止按钮 / `done_toolbar` / 稳定性计数）：按固定间隔做（它很便宜，不需要文本提取）。

这样在"长时间无变化"的站点上能真正省下周期性 `innerHTML → Markdown` 的开销。
风险：稳定性计数的"拍"语义会变（需要按时间而非拍数计量），必须先设计再改。
