...
## 结束信号：网络请求结束 + 停止按钮（2026-09 实测）

### 网络跟踪（主要结束信号）

配置 `network.url_pattern` 后，`WebChatProvider` 自动注入 XHR + fetch 拦截脚本：
- 匹配 URL 模式的请求完成后，`window._aiw2a_done` 置 `true`
- `_poll_response_net` 检测到请求完成 → 立即 DOM 兜底提取全文
- **不再依赖 `stable_polls` 稳定性计数**，定稿延迟从 ~8s 降到请求结束 + 0.5s

### 停止按钮

生成中输入框右下角出现停止按钮，生成结束按钮消失。已加入 `selectors.stop_button` 候选。

### 历史背景（2026-09-22 旧实测，已过时）

| 候选 | 旧实测结果 |
|---|---|
| 停止按钮 | ❌ 不存在（`button[aria-label*="停止"]`、`[class*="stop-btn"]` 全空） |
| 助手消息工具栏（复制/点赞/朗读…） | ❌ **不存在**：hover 到回答上前后节点数 `7→7`、`button` 数 `0→0` |
| `#to-bottom-button`（含 `loading-i3Fu5w`） | ⚠️ 生成中可见、结束消失，但语义跟随滚动，不可靠 |
| `dot-flashing`（三点动画） | ⚠️ 只在"思考/加载"阶段出现，正文开始后即消失 |

旧结论：**豆包的快速结束信号只能来自网络侧**（"完成请求的响应流结束"事件）或继续用稳定性兜底。
