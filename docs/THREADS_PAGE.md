...
> 状态：**已实现**（提交 `54a922e` 后端参数 / `063ddba` 独立页 / `cbe304a` 状态面板瘦身 / 侧栏服务端搜索）。
...
1. **分页**：`offset/limit` + 「加载更多」，并显示「已加载 X / 共 Y 条」。✅
2. **状态面板**：移除会话表格，改「活跃 N / M · 共 T 条」摘要 + 「查看全部 →」；KPI「活跃会话」可点。✅
3. **搜索**：服务端过滤（`q` 命中 title/thread_id/provider/model），Playground 侧栏一并切服务端。✅
4. **批量清理**：本次**不做**（仅单条销毁）。
5. **每页条数**：20 / 50 / 100 三档。✅

### 实测
- 会话页：共 36 条 → 首屏 20，加载更多后 36（无重复）；搜索「天气」8 条；过滤 chatgpt 12 条；无结果显示空状态；「继续」跳转 `/ui/playground.html?thread_id=…` 且输入框已填。
- 状态面板：表格消失，摘要「活跃 0 / 8 · 共 36 条」，KPI 点击进入会话页。
- Playground 侧栏：搜索「天气」→ 8 条（**服务端过滤**，此前是本地过滤会漏），无匹配有文案；超过 50 条时显示「仅显示最近 50 / 共 N 条 · 会话页 →」。
