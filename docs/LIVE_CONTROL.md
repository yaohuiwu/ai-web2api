...
**信任边界**（同 `docs/LIVE_VIEW.md` 提示）：服务**默认只监听 `127.0.0.1`**，Docker 也把宿主端口**只绑回环**（`127.0.0.1:8000:8000`）→ 局域网/公网默认访问不到。
一旦你把它暴露出去（`WEB2API_HOST=0.0.0.0` + 改端口绑定），**同网络可达者都能操作这个浏览器**
