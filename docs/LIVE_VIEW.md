...
| 症状 | 真因 | 修法 |
|---|---|---|
| 画面"很快就断了" | 采集任务每拍都会走到"空闲兜底"分支，而该分支调用的 `_capture_into()` 在重构时被删掉 → 空闲 ~5s 后任务 `AttributeError` 崩溃，观众侧停更 | 补回方法 + 测试守住 |
| 画面反复"断开又重连" | 前端 `onerror` 把 `<img>` 整个删掉，状态轮询（2.5s）又重建 → 断/连循环 | 出错**不删 img**，只显示提示；重试**指数退避**（3s→15s） |
| 同页两条流互抢导致画面被打断 | screencast 是**每页一份**，而流按 (provider, thread) 各一份 | 记录 screencast 归属：同页已有在跑的流 → 本流**自动回退定时截图**（日志可见），且**失败不再每拍重试** |
| 空闲久了画面像冻住 | CDP 只在重绘时推帧 | 空闲兜底 keepalive（`live_idle_keepalive_seconds`，默认 5s 补一帧）；状态栏显示**帧龄** |
