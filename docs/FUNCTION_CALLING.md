...
- 工具轮次每轮都带 `tools`，但 resume **不重复注入工具说明**：页面上下文里之前 create 时已经有一份，
  再发一次是浪费（这也避免了“工具说明发两遍”）。仅当页面是新会话（create）时才注入。
- **客户端契约**：工具信息必须在**模型可见的上下文**里——**无状态 / create** 时由当前请求的 `tools` 注入；
  **resume** 不注入、依赖页面历史。所以同一 thread 要用工具，必须在**某次 create 请求**里带过 `tools`（通常是首轮）；
  只在 resume 才带、之前从没带过 → 页面无说明可复用，模型可能不以规定格式调用。这是**客户端责任**，网关不代做补发。
